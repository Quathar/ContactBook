...
import com.quathar.app.gui.component.ChangerComboBox;
 ...
 * ContactInfo.<br><br>
 * 
 * Clase que muestra gr�ficamente toda la informaci�n de un contacto.
 *
 * @since 2022-04-17
 * @see DAO
 * @see HobbyTable
 ...
 * @author Q
 */
public class Info extends JFrame implements ActionListener { // CLASE FINALIZADA
	 ...
	 * Icono de los campos.
	 */
	private final String[] IconTitle = {"\uE13D", "\uE1C3", "\uE13D", "\uE163", "\u26A5"}; // Nombre, Direcci�n, Apellidos, Fecha_nac, G�nero
	/**
	...
	/**
	 * T�tulo de los botones CE (Cerrar, Editar).
	 */
	private final String[] CETitle = {"CERRAR", "EDITAR"};
	/**
	 * T�tulo de los botones VG (Volver, Guardar).
	 */
	...
	/**
	 * Componente JComboBox<String> para seleccionar el g�nero.
	 */
	...
	//--TEL�FONOS Y CORREOS
	/**
	 * Componente TelephoneTable de los tel�fonos.
	 */
	private TelephoneTable tTable;
	/**
	 * Componente JTextField para los tel�fonos.
	 */
	private JTextField txtTelefono;
	/**
	 * Componente JComboBox<String> para seleccionar el tipo de tel�fono.
	 */
	private JComboBox<String> tType;
	/**
	 * Botones AD (Add, Delete) de tel�fonos.
	 */
	 ...
	 */
	public Info(ContactBook contactBook, DAO dao, int id) {
		super("Informaci�n del contacto");
	...
	// M�TODOS
	/**
	 * Obtiene informaci�n del contacto no referente a las tablas.
	 */
	...
	// <<ZONA DE DISE�O>>
	/**
	 * Crea los componentes y dise�a el frame.
	 */
	private void initComponents() {
		setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		setBounds(	(int) (ContactBook.ScreenSize.width * 0.35),	// X position
					(int) (ContactBook.ScreenSize.height * 0.3),	// Y position
					(int) (ContactBook.ScreenSize.width * 0.3),	// Width
					(int) (ContactBook.ScreenSize.height * 0.45));	// Height
		setResizable(false);
	...
	/**
	 * Dise�a y crea los componentes el <b>panel info</b>.
	 */
	private void info() {
		...
		gbl = ContactBook.createGridBagLayoutFull(2, 2);
	...
	/**
	 * Da un valor al �ndice del desplegable g�nero.
	 * 
	...
	/**
	 * Dise�a y crea los componentes del <b>panel formulario</b>.<br><br>
	 * 
	 * El dise�o y el n�mero de componentes var�a en funci�n del tipo de contacto.
	 * 
	...
	private void createFormulario(JPanel info) {
		...
		gbl = ContactBook.createGridBagLayout(gblSize[0], gblSize[1], new int[] {1}, new int[] {gblSize[1] - 1});
		form.setLayout(gbl);
		...
		gbc = ContactBook.createGridBagConstraints(0, rowNumber);
		form.add(lblNombre, gbc);
		
		nameTextField = new JTextField();
		nameTextField.setText(c.getName());
		nameTextField.setEditable(false);
		gbc = ContactBook.createGridBagConstraints(1, rowNumber++);
		if (tipo.equals(Contact.Type[0]))
			gbc.gridwidth = 3;
		...
		if (tipo.equals(Contact.Type[0])) {
			...
			gbc = ContactBook.createGridBagConstraints(2, rowNumber);
			...
			gbc = ContactBook.createGridBagConstraints(3, rowNumber);
	...
	/**
	 * Dise�a y crea los componentes del <b>panel aficiones</b>.
	 * 
	 * @param info the parent panel
	 */
	private void createAficiones(JPanel info) {
		...
		gbc = ContactBook.createGridBagConstraints(x, y);
		if (tipo.equals(Contact.Type[2]))
			gbc.gridheight = 2;
		gbc.fill = GridBagConstraints.BOTH;
	...
	/**
	 * Dise�a y crea los componentes del <b>panel tables</b>.
	 * 
	 * @param info the parent panel
	 */
	private void createTables(JPanel info) {
		...
		gbl = ContactBook.createGridBagLayoutFull(4, 1);
		...
		gbl = ContactBook.createGridBagLayoutFull(3, 0);
		...
		for (int i = 0; i < ADTitle.length; i++) {
			...
			gbc = ContactBook.createGridBagConstraints(i + 1, 0);
			gbc.fill = GridBagConstraints.HORIZONTAL;
	...
	/**
	 * Dise�a y crea los componentes de los paneles.<br><br>
	 * 
	...
	private boolean listenerTables(Object event) {
		// AFICIONES
		if (btnADaficiones[0] == event) {						// BOT�N ADD AFICIONES
			...
			return true;
		} else if (btnADaficiones[1] == event) {				// BOT�N DELTE AFICIONES
		...
		// TEL�FONOS
		if (btnADtelefonos[0] == event || txtTelefono == event) {// BOT�N ADD TEL�FONOS
			...
			txtTelefono.setText("");
			return true;
		} else if (btnADtelefonos[1] == event) {				// BOT�N DELTE TEL�FONOS
			tTable.deleteSelectedRows();
			return true;
		}
		
		// CORREOS
		if (btnADcorreos[0] == event || txtCorreo == event) {	// BOT�N ADD CORREOS
			...
			txtCorreo.setText("");
			return true;
		} else if (btnADcorreos[1] == event) {					// BOT�N DELTE CORREOS
	...
	/**
	 * Habilita o deshabilita todos los componentes de las tablas tel�fonos y correos.
	 * 
	...
	private boolean listenerCE(Object event) {
		if (btnCE[0] == event) {		// BOT�N CERRAR
			dispose();
			return true;
		} else if (btnCE[1] == event) {	// BOT�N EDITAR
	...
	private boolean listenerVG (Object event) {
		if (btnVG[0] == event) {		// BOT�N VOLVER
			String out = "�Est� seguro de que quiere volver?\n"
					+ "NO se guardar�n los cambios";
			...
			return true;
		} else if (btnVG[1] == event) {	// BOT�N GUARDAR
