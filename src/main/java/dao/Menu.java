 ...
 * Sólo permite los aspectos más básicos, NO se pueden agregar ni añadir (a un contacto) aficiones.
 *
 * @since 2022-04-05
 * @see Database
 * @see IO
...
public class Menu { // CLASE FINALZIADA
	 ...
	 * Constructor.
	 * 
	 * @param db agenda a la que se implementa este menú
	 */
	...
	private boolean register() {
		...
		if (c.getType().equals(Contact.Type[0]) || c.getType().equals(Contact.Type[1])) {
			// TELÉFONOS
			IO.println("CUÁNTOS TELÉFONOS DESEA AÑADIR ?");
			int size = IO.readInt();
			if (size > 0) {
				...
				for (int i = 0; i < size; i++) {
					...
					while (tNumber.equals("ERROR")) {
						IO.println("Introduzca 9 números");
						tNumber = RegexFilter.checkTelephone(IO.readStringNotBlank());
					}
					IO.println("TIPO ? (Móvil | Fijo)");
			...
			// CORREOS
			IO.println("CUÁNTOS CORREOS DESEA AÑADIR ?");
			size = IO.readInt();
			if (size > 0) {
	...
	// <<CONSULTAR>>
	/**
	 * Devuelve la información de un contacto a partir de su ID.
	 * 
	 ...
	 * 
	 * Si el campo se deja vacío se pone el antiguo valor que tuviera.
	 * 
	...
	private boolean modify() {
		...
		if (type.equals("error"))
			...
			IO.println("DESEA MODIFICAR ALGÚN TELÉFONO ? (SI | NO)");
			while (modifyTelephones());
			IO.println("DESEA MODIFICAR ALGÚN CORREO ? (SI | NO)");
			while (modifyMails());
			return db.update(c);
		}
	}
	...
	/**
	 * Modifica los teléfonos de un contacto (Si tuviera).
	 * 
		...
		switch (selection) {
			case 'S':
				while (selection == 'S') {
					...
					IO.println("DESEA MODIFICAR OTRO TELÉFONO (SI | NO) ?");
					if (IO.readUpperChar() != 'S') {
						c.setTelephones(telephones);
						return false;
					}
					IO.println("CUANDO ACABE SE REFLEJARÁN LOS CAMBIOS, >>>NO REPITA ID<<<");
				}
			case 'N':
				c.setTelephones(telephones);
				return false;
			default:
				IO.println("Seleccione una opción válida");
				return true;
		}
	}
	...
	// <<SELECCIONAR TIPO>>
	/**
	 * Selecciona el tipo de contacto que el usuario desea añadir.
	 * 
	...
	// <<<MENÚ>>>
	/**
	 * Menú interactivo para que el usuario pueda manipular la BBDD.
	 * 
